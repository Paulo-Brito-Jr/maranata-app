...
import { ImpersonarStarter, ImpersonarSair } from "./impersonar-controls";
...
export function AdminTopbar({
  ...
  // Está impersonando quando o role real é SUPER_ADMIN mas o role aparente
  // é outro (cookie de impersona).
  const impersonando = ehSuperReal && user?.role && user.role !== "SUPER_ADMIN";
  ...
  return (
    <header className="flex h-16 items-center justify-between border-b border-border bg-background/60 px-6 backdrop-blur">
      ...
      <div className="flex items-center gap-3">
        ...
        {ehSuperReal && !impersonando && <ImpersonarStarter />}
        <ThemeToggle />
        <div className="hidden md:flex flex-col items-end leading-tight">
          <span className="text-sm font-medium">{user?.name ?? "—"}</span>
          <span className="text-xs text-muted-foreground">{user?.email ?? ""}</span>
        </div>
        {impersonando && <ImpersonarSair texto="↩ Voltar a Super Admin" />}
